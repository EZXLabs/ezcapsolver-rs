...
/// Blocking client for asynchronous and synchronous EzCaptchaSolver API operations.
#[derive(Clone)]
pub struct EzCapSolverClient {
    client_key: String,
...
impl fmt::Debug for EzCapSolverClient {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("EzCapSolverClient")
...
impl EzCapSolverClient {
    ...
    pub fn with_http_client(config: ClientConfig, http: reqwest::blocking::Client) -> Result<Self> {
        let client_key = config.resolve_client_key()?;
        let client_key_header = client_key_header(&client_key)?;
        config.validate()?;
        Ok(Self {
    ...
    pub fn create_task<N, T>(&self, task_type: N, task: &T) -> Result<CreateTaskResponse>
    where
        N: TaskTypeName,
        T: Serialize + ?Sized,
    {
        ...
        let response: CreateTaskResponse = self.post(
    ...
    pub fn get_task_result(&self, task_id: &str) -> Result<TaskResult> {
    ...
    pub fn wait_for_result(&self, task_id: &str) -> Result<TaskResult> {
    ...
    pub fn wait_for_result_with(
        ...
        for attempt in 1..=polling.max_attempts {
            ...
            let result = match self.get_task_result(task_id) {
                Ok(result) => result,
    ...
    pub fn solve<N, T>(&self, task_type: N, task: &T) -> Result<Solved<Solution>>
    where
        N: TaskTypeName,
        T: Serialize + ?Sized,
    {
        self.solve_with(task_type, task, self.config.polling)
    }

    /// Creates an asynchronous task and uses per-call polling settings.
    pub fn solve_with<N, T>(
        &self,
        task_type: N,
        task: &T,
        polling: PollingConfig,
    ...
    where
        ...
        T: Serialize + ?Sized,
    {
        let created = self.create_task(task_type, task)?;
    ...
    pub fn sync_solve<N, T>(&self, task_type: N, task: &T) -> Result<Solved<Solution>>
    where
        N: TaskTypeName,
        T: Serialize + ?Sized,
    {
        let result = self.create_sync_task(task_type, task)?;
    ...
    pub fn create_sync_task<N, T>(&self, task_type: N, task: &T) -> Result<TaskResult>
    where
        N: TaskTypeName,
        T: Serialize + ?Sized,
    {
        ...
        let result: TaskResult = self.post(
    ...
    pub fn balance(&self) -> Result<f64> {
        ...
        let response: BalanceResponse = self.post(
    ...
    fn post<B, T>(&self, url: &str, body: &B, timeout: Duration) -> Result<T>
    where
        B: Serialize + ?Sized,
        T: DeserializeOwned,
    {
