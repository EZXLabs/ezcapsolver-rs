use super::AsyncEzCapSolverClient;
use crate::{
...
macro_rules! define_solver {
    ($mode:ident, $name:ident, $task:ty, $task_type:ident, $solution:ty) => {
        ...
        pub async fn $name(&self, task: &$task) -> Result<Solved<$solution>> {
            let solved = self.solve(TaskType::$task_type, task).await?;
        ...
        pastey::paste! {
            ...
            pub async fn [<sync_ $name>](&self, task: &$task) -> Result<Solved<$solution>> {
                let result = self.create_sync_task(TaskType::$task_type, task).await?;
